...
export default function CampaignsPage() {
    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                const { data: { status, data, message } } = await http.get('campaigns', {
                    params: {
                        page: 1
                    }
                })
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between items-start">
                <h1 className="text-2xl text-slate-800 font-semibold mb-6">Campaign List</h1>
                {/* <div>
                    <Link
                        href="/campaigns/add"
                        className={buttonVariants({ variant: "default", className: "flex items-center gap-x-1" })}
                    >
                        <PlusIcon className="w-4 h-4" /> Add Campaign
                    </Link>
                </div> */}
            </div>
